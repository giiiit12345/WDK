...
void DrawHalfEllipse(wxMemoryDC& memDC, int x, int y, int radiusX, int radiusY, const wxColour& borderColor, bool drawLeftHalf)
{
    ...
    if (gc)
    {
        ...
        // 根据 `drawLeftHalf` 参数决定绘制左半部分还是右半部分
        if (drawLeftHalf)
        {
            // 裁剪区域为椭圆的右半部分
            gc->Clip(wxRect(x - radiusX, y + radiusY, x, y - radiusY));
        }
        else
        {
            // 裁剪区域为椭圆的左半部分
            gc->Clip(wxRect(x, y + radiusY, x + radiusX, y - radiusY));
        }

        gc->DrawEllipse(x, y, 2 * radiusX, 2 * radiusY);
        ;
