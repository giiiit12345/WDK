...
void Board::SaveToJson(const std::string& filePath) {
    ...
    for (const auto& item : componentList) {
        ...
        std::string componentIdWithNum = "component" + componentIdStr + "_" + std::to_string(componentCounter[componentIdStr]);


        // 元器件 ID
        componentJson["id"] = componentIdWithNum; // 更改为使用 componentIdWithNum
        ...
        // 边界框（bounding box）
        Json::Value borderJson(Json::arrayValue);
        for (int i = 0; i < 4; i++) {
            borderJson.append(component->border[i]);
        }
        componentJson["boundingBox"] = borderJson;
