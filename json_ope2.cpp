...
void DrawFromJson(Json::Value root, wxMemoryDC& memDC, int x, int y)
{
    ...
    //绘制连接点
    for (const Json::Value& functionData : functions["point"]) {
        int x1 = functionData["factor"][0].asInt();
        int y1 = functionData["factor"][1].asInt();

        DrawPoint(memDC, x + x1, y + y1);
    ...
    //绘制弧线
    for (const Json::Value& functionData : functions["curve"]) {

        if (functionData["factor"].size() < 8) {
            std::cerr << "Invalid JSON data: factor array size is too small." << std::endl;
            continue;
        }
        ...
        int x3 = functionData["factor"][4].asInt();
        int y3 = functionData["factor"][5].asInt();
        int x4 = functionData["factor"][6].asInt();
        int y4 = functionData["factor"][7].asInt();

        // 读取绘制工具相关参数
        wxColour borderColor = ReadColorFromJson(functionData["pen"]["color"]);

        DrawCurve(memDC, x + x1, y + y1, x2 + x, y + y2, x + x3, y + y3, x + x4, y + y4, borderColor);
    }
}
